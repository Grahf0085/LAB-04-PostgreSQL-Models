...
export class Animorph {
  ...
  static async findByid(id) {
    const { rows } = await pool.query(`
      SELECT id, name, morph, minutes, is_morphed as "isMorphed" 
      FROM animorph 
      WHERE id = $1`
    , [id]
    );
  ...
  static async update(id, { name, morph, minutes, isMorphed }) {
    const { rows } = await pool.query(`
      UPDATE animorph SET name = $2, morph = $3, minutes = $4, is_morphed = $5
      WHERE id = $1 
      RETURNING id, name, morph, minutes, is_morphed as "isMorphed"`,
    [id, name, morph, minutes, isMorphed]
    );
